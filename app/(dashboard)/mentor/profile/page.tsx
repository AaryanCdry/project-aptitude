  ...
  return (
    <div className="max-w-2xl mx-auto p-8">
      ...
      <div className="bg-surface-container-lowest border border-outline-variant rounded-xl p-6 shadow-sm mb-6 flex items-center gap-6">
        <div className="size-20 rounded-full bg-secondary-fixed flex items-center justify-center shrink-0">
          <span className="text-on-secondary-fixed font-display-sm text-2xl font-bold">{initials}</span>
        </div>
        <div>
          <h2 className="font-headline-md text-on-surface text-xl font-bold">{profile?.name ?? 'Mentor'}</h2>
          <p className="font-body-md text-on-surface-variant">{profile?.email}</p>
          <div className="flex items-center gap-2 mt-2">
            <span className="bg-secondary-fixed text-on-secondary-fixed-variant text-xs font-bold px-2.5 py-1 rounded-full">{profile?.role}</span>
