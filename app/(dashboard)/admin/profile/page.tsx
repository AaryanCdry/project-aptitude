...
export default async function AdminProfilePage() {
  ...
  const initials = getInitials(profile?.name ?? profile?.email ?? 'A');
    ...
    <div className="max-w-2xl mx-auto">
      ...
      <div className="bg-surface-container-lowest border border-outline-variant rounded-xl p-6 shadow-sm mb-6 flex items-center gap-6">
        <div className="size-20 rounded-full bg-primary flex items-center justify-center shrink-0">
          <span className="text-on-primary font-display-sm text-2xl font-bold">{initials}</span>
        </div>
        <div>
          <h2 className="font-headline-md text-on-surface text-xl font-bold">{profile?.name ?? 'Admin'}</h2>
